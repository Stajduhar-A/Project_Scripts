function automatingSemanticFeat(subnum, run)
    ...
    while ~feof(fin)
        ...
        %% we need to change the output file, the input file, the anatomical, and the 5 ptr sheets
          ...
          s=strrep(s,['Semantic' num2str(run-1)], ['Semantic' num2str(run)]); % change output folder 

    
        s=strrep(s,['semantic_run-' num2str(run-1)], ['semantic_run-' num2str(run)]); % change the functional run input
        ...
        s=strrep(s,['sub-0' num2str(subnum-1) '/anat/' 'sub-0' num2str(subnum-1)],['sub-0' num2str(subnum) '/anat/' 'sub-0' num2str(subnum)]); % change anatomical scan but only every 5 while loops
             ...
             s=strrep(s,['semantic_ptr_run' num2str(run-1)],['semantic_ptr_run' num2str(run)]); %% intact. S4, S16, S64, S256 PTR sheet
